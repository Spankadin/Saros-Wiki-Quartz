---
title: Welcome to Saros
description: A player-facing guide to the people, places, factions, and history of Saros.
aliases:
  - Home
tags:
  - Lore
  - Published
share: true
---

# Delve into Saros

Saros is a realm of ancient ruins, dangerous roads, powerful factions, and communities built atop the remains of older worlds. This wiki gathers the player-facing lore of the setting into one place.

> [!tip] Start here
> New to the setting? Begin with [[A Brief Saros History]], then explore [[Ages of Saros]] and the [[Sarosian Timeline]]. Hover over a link for a preview, use search for a specific name, or follow the graph to see how the world connects.

## Explore the Realm

- [[Major Cities, Towns, and Ports|Cities, towns, and ports]]
- [[Landmarks]]
- [[Saros - Distances, Travel, Trade, and Population|Travel, trade, and population]]
- [[Griffin Riders]]

## Powers and Factions

- [[The Crown of Saros]]
- [[The Aegis Legion]]
- [[The Prism]]
- [[The Black Circle]]
- [[Dwarven Clans]]
- [[Gambler's Guild]]
- [[Merchant's Guild]]

## People of Saros

- [[NPCs|People and notable figures]]
- [[Queen Amara]]
- [[Gerdga]]
- [[Malgrim Darkshade]]

> [!info] Join the table
> A secure Foundry VTT link will be added here when the `play` subdomain is connected.

This site publishes only notes deliberately marked `share: true` in the private Saros Obsidian vault.
